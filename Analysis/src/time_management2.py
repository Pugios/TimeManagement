...
class TimeManagement:
  ...
  def load_files(self):
    ...
    if exists(process_tags_path):
      ...
      unnamed = [c for c in process_tags.columns if c.startswith("Unnamed")]
      if unnamed:
      ...
      if "Process" not in process_tags.columns:
        cols = list(process_tags.columns)
        # try to identify by matching values with app.Process
        app_processes = set(app.Process.unique())
        best_col = None
        best_match = 0
        for c in cols:
          matches = sum(1 for v in process_tags[c].astype(str) if v in app_processes)
          if matches > best_match:
            best_match = matches
            best_col = c
        if best_col:
          process_tags = process_tags.rename(columns={best_col: 'Process'})
        else:
          # fallback: assume first column is Process
          process_tags = process_tags.rename(columns={cols[0]: 'Process'})
