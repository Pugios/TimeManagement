...
class TimeManagement:
  ...
  def all_time(self):
    ...
    grouped_df = self.app.groupby(["Effective_Day", "Tag"])["Duration"].sum().reset_index()
    ...
    weekly_df = pivot_df.resample("W-MON").sum()
    ...
    weekly_df.plot(kind="line", grid=True, color=self.colors, ax=ax)
    ax.set_ylabel("Hours")
    ax.set_xlabel("Week")
    ax.set_title("All-time weekly totals")
    ax.legend(loc="upper left")
    plt.xticks(rotation=45, ha="right")

    fig.canvas.manager.set_window_title("Time Management")
    fig.tight_layout()
    plt.savefig(self.save_path + "/all_time_weekly.jpg")
    return ax
